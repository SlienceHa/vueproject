...
def main(args):
    ...
    """
    graphs = args.input_path + "0.json"
    data = json.load(open(graphs))
    docVectors=[]
    id=[]
    if data['exit']=='1':
        with open('./pythonCode/features/vectorChild.csv','r') as fr:
            data2=csv.reader(fr)
            index=0
            for i in data2:
                if index!=0:
                    ...
                    if i[0]==data['name']:
                        ...
                        break
                index += 1
        with open('./pythonCode/features/modelVectors.json','w') as fw:
            json.dump({"vector":docVectors[0],'id':id,'name':data['name']},fw)
    else:
        document_collections = feature_extractor(graphs, args.wl_iterations)
        model = Doc2Vec.load('./pythonCode/features/treeGraph2vecChild.model')
        docVectors=model.infer_vector(document_collections,epochs=100)
        with open('./pythonCode/features/modelVectors.json','w') as fw:
            json.dump({"vector":docVectors.tolist()},fw)
